...
const Summaries = ({ onSendMessage, onEditMessage, onCancelGeneration, onBackToDashboard, sidebarOpen, messages = [], processingStates = {} }) => {
  ...
  const handleSendMessage = (message, files = [], images = []) => {
    onSendMessage(message, files, images, 'summaries');
  };
  ...
  const handleCancelEdit = () => {
    ...
    if (messageIdToReuse && originalTextToReuse && originalTextToReuse.trim()) {
      onEditMessage(messageIdToReuse, originalTextToReuse, 'summaries');
    }
  };

  const handleSaveEdit = () => {
    if (editingText.trim() && editingMessageId) {
      onEditMessage(editingMessageId, editingText, 'summaries');
      setEditingMessageId(null);
      setEditingText('');
    }
  };
  ...
  return (
    <div className="summaries-dashboard">
      {!hasMessages ? (
        <>
          {/* Header */}
          <div className="dashboard-header">
            <div className="header-left">
              <div className="summaries-content">
                <h1 className="summaries-title">Olá!</h1>
                <p className="welcome-subtitle">Vamos criar resumos inteligentes?</p>
              </div>
            </div>
          </div>
