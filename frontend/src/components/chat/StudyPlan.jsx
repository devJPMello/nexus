...
const StudyPlanDashboard = ({ onSendMessage, onEditMessage, onCancelGeneration, onBackToDashboard, sidebarOpen, messages = [], processingStates = {} }) => {
  ...
  const handleSendMessage = (message, files = [], images = []) => {
    onSendMessage(message, files, images, 'study-plan');
  };
  ...
  const handleCancelEdit = () => {
    ...
    if (messageIdToReuse && originalTextToReuse && originalTextToReuse.trim()) {
      onEditMessage(messageIdToReuse, originalTextToReuse, 'study-plan');
    }
  };

  const handleSaveEdit = () => {
    if (editingText.trim() && editingMessageId) {
      onEditMessage(editingMessageId, editingText, 'study-plan');
      setEditingMessageId(null);
      setEditingText('');
    }
  };
          ...
          <div className="dashboard-header">
            <div className="header-left">
              <div className="plan-content">
                <h1 className="studyplan-title">Olá!</h1>
                <p className="welcome-subtitle">Vamos organizar seu plano de estudos?</p>
              </div>
            </div>
          </div>
